...
export default function TerminalWindow({ fileSystem, setFileSystem, onClose }) {
  ...
  const printPrompt = useCallback((cwdRef) => {
    commandRef.current = "";
    if (xtermRef.current) {
      const user = colorText("orbit", "green");
      const sep = colorText(` ${cwdRef.current}`, "magenta");
  ...
  useEffect(() => {
    ...
    const handleCommand = (cmd) => {
      ...
      if (!base) return printPrompt(cwdRef);
      ...
      if (base === "cpp") {
        if (!args[0]) {
          xtermRef.current.writeln("Usage: cpp <filename.cpp>");
          return printPrompt(cwdRef);
        }

        // NEW: Check for and display initialization errors
        if (cppCompiler.initError) {
          xtermRef.current.write(
            colorText(
              `\nC++ Compiler failed to load: ${cppCompiler.initError}\n`,
              "red"
            )
          );
          return printPrompt(cwdRef);
        ...
        if (!cppCompiler.isReady) {
          xtermRef.current.write(
            colorText(
              "C++ compiler is still loading, please wait...\n",
              "yellow"
            )
          );
          return printPrompt(cwdRef);
      ...
      if (isAnyExecuting()) {
        if (domEvent.key === "Enter") {
          ...
          if (isWSExecuting) sendWSInput(inputBuffer.current + "\n");
            ...
            cppCompiler.sendInput(inputBuffer.current);
          inputBuffer.current = "";
