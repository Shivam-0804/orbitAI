...
export default function TerminalWindow({
  ...
  useEffect(() => {
    const getAllStatePaths = (node, paths = {}) => {
        paths[node.path] = { type: node.type, content: node.content };
        if (node.children) {
            for (const child of node.children) getAllStatePaths(child, paths);
        }
        return paths;
    };
    const getAllLfsPaths = async (dir, paths = {}) => {
        try {
            ...
            for (const entry of entries) {
                ...
                if (stat.isDirectory()) {
                    paths[path] = { type: 'folder' };
                    await getAllLfsPaths(path, paths);
                } else {
    ...
    const syncFileSystem = async () => {
        ...
        if (!rootNode) return;
        const statePaths = getAllStatePaths(rootNode);
        const lfsPaths = await getAllLfsPaths('/');
        const pathsToDelete = Object.keys(lfsPaths).filter(p => !statePaths[p]);
        pathsToDelete.sort((a, b) => b.length - a.length);
        for (const path of pathsToDelete) {
            if (lfsPaths[path].type === 'file') await pfs.unlink(path).catch(console.error);
            else if (lfsPaths[path].type === 'folder') await pfs.rmdir(path).catch(console.error);
        }
    ...
    syncFileSystem().catch(console.error);
  }, [fileSystem]);

  const isAnyExecuting = useCallback(() => isWSExecuting || pyodide.isExecuting || pyodide.isInstalling || cppCompiler.isExecuting, [ isWSExecuting, pyodide.isExecuting, pyodide.isInstalling, cppCompiler.isExecuting ]);
  const safeWrite = (term, data) => { if (term && !term.isDisposed) term.write(data); };
  const safeWriteln = (term, data = "") => { if (term && !term.isDisposed) term.writeln(data); };
  const colorText = (text, color) => { const colors = { red: "\x1b[31m", green: "\x1b[32m", yellow: "\x1b[33m", blue: "\x1b[34m", magenta: "\x1b[35m", cyan: "\x1b[36m", white: "\x1b[37m", reset: "\x1b[0m" }; return `${colors[color] || ""}${text}${colors.reset}`; };

  const printPrompt = useCallback((terminalId) => {
  ...
  const handleNewTerminal = () => { const newIdVal = nextId.current++; const newTerminal = { id: newIdVal, name: `orbit ${newIdVal}`, cwd: "/" }; setTerminals((prev) => [...prev, newTerminal]); setActiveTerminalId(newIdVal); };
  const handleKillTerminal = (idToKill) => { if (terminals.length <= 1) return; const instance = xtermInstances.current[idToKill]; if (instance) { instance.term.dispose(); instance.container.remove(); delete xtermInstances.current[idToKill]; } delete commandBuffers.current[idToKill]; delete inputBuffers.current[idToKill]; const newTerminals = terminals.filter((t) => t.id !== idToKill); setTerminals(newTerminals); if (activeTerminalId === idToKill) setActiveTerminalId(newTerminals[0]?.id || null); };
  const handleResetTerminal = useCallback(() => { resetTerminalState({ setTerminals, setActiveTerminalId, setActiveView, setInstalledPackages, xtermInstancesRef: xtermInstances, inputBuffersRef: inputBuffers, commandBuffersRef: commandBuffers, nextIdRef: nextId }); }, []);
  useEffect(() => { if (resetTerminalRef) resetTerminalRef.current = { reset: handleResetTerminal }; }, [resetTerminalRef, handleResetTerminal]);
  useEffect(() => { const container = terminalContainerRef.current; if (!container) return; const resizeObserver = new ResizeObserver(() => { const activeId = activeTerminalIdRef.current; const activeInstance = xtermInstances.current[activeId]; if (activeInstance?.fitAddon) try { activeInstance.fitAddon.fit(); } catch (e) { console.error(e); } }); resizeObserver.observe(container); return () => resizeObserver.disconnect(); }, []);
  ...
  useEffect(() => {
    terminals.forEach(({ id }) => {
      if (!xtermInstances.current[id] && terminalContainerRef.current) {
        ...
        const term = new Terminal({ cursorBlink: true, fontSize: 14, theme: { background: "#181818", foreground: "#ffffff" }, scrollback: 1000, convertEol: true });
      ...
      const localCommands = {
        ...
        cd: () => {
          ...
          if (folder && folder.type === "folder") {
            setTerminals((prev) => prev.map((t) => t.id === activeTerminalId ? { ...t, cwd: newPath } : t));
          } else { return colorText(`cd: no such directory: ${args[0]}`, "red"); }
          return "";
        },
        mkdir: async () => {
          if (!args[0]) return;
        ...
        rm: async () => {
          ...
          if (!node) { return colorText(`rm: cannot remove '${args[0]}': No such file or directory`, "red"); }
          if (node.type === "file") { await pfs.unlink(path); } 
          else if (node.type === "folder") {
              try { await pfs.rmdir(path); } 
              catch (e) { return colorText(`rm: cannot remove '${args[0]}': Directory not empty`, "red"); }
          }
        ...
        git: async () => {
          if (!args[0]) { safeWriteln(term, "Usage: git <command> [options]"); return; }
          const gitCmd = args[0];
          const onAuth = () => { if (!credentials) { safeWriteln(term, colorText("Authentication required. Run 'login'", "yellow")); return; } return { username: credentials.username, password: credentials.token }; };
          const isRepo = await (async () => { try { await pfs.stat(`${currentCwd}/.git`); return true; } catch (e) { return false; } })();
          if (!['init', 'clone'].includes(gitCmd) && !isRepo) { safeWriteln(term, colorText(`fatal: not a git repository`, "red")); return; }
          
          try {
            switch (gitCmd) {
                 case "clone":
                  if (!args[1]) { safeWriteln(term, "Usage: git clone <url> [directory]"); break; }
                ...
                case "status": {
                  ...
                  if (staged.length) { safeWriteln(term, '\r\nChanges to be committed:'); staged.forEach(s => safeWriteln(term, `\t${colorText(`${s.type}:      ${s.file}`, 'green')}`)); }
                  if (modified.length) { safeWriteln(term, '\r\nChanges not staged for commit:'); modified.forEach(m => safeWriteln(term, `\t${colorText(`${m.type}:  ${m.file}`, 'red')}`)); }
                  if (untracked.length) { safeWriteln(term, '\r\nUntracked files:'); untracked.forEach(u => safeWriteln(term, `\t${colorText(u, 'red')}`)); }
                  break;
                }
                case "add":
                  if (args.length < 2) { safeWriteln(term, colorText("Nothing specified, nothing added.", "red")); break; }
                ...
                case "commit": {
                  ...
                  if (msgIndex === -1 || !args[msgIndex + 1]) { safeWriteln(term, colorText('Usage: git commit -m "message"', 'red'));
                  } else {
                ...
                case "branch": {
                  ...
                  if (M_flag_index !== -1 && args[M_flag_index + 1]) {
                    ...
                    if (!args[2]) { safeWriteln(term, "Usage: git branch -d <branch-name>"); break; }
                ...
                case "checkout":
                  if (!args[1]) { safeWriteln(term, "Usage: git checkout <branch-name>"); break; }
                ...
                case "fetch":
                  ...
                  break;
                case "merge":
                  if (!args[1]) { safeWriteln(term, "Usage: git merge <branch-name>"); break; }
                  const mergeResult = await git.merge({ fs: lfs, dir: currentCwd, theirs: args[1], author: { name: "OrbitAI", email: "orbit@ide.com" } });
                  if (mergeResult.conflicts) { safeWriteln(term, colorText("Automatic merge failed; fix conflicts and then commit the result.", "red")); }
                  else { safeWriteln(term, "Merge successful."); }
                  break;
                case "rebase":
                  if (!args[1]) { safeWriteln(term, "Usage: git rebase <branch-name>"); break; }
                  ...
                  break;
                case "reset":
                  if (!args[1]) { safeWriteln(term, "Usage: git reset [--hard|soft|mixed] <commit-sha>"); break; }
                  let mode = 'mixed'; let ref = args[1];
                  if (['--hard', '--soft', '--mixed'].includes(args[1])) { mode = args[1].substring(2); ref = args[2]; }
                  if (!ref) { safeWriteln(term, "Error: missing commit reference for reset."); break; }
                ...
                case "remote":
                  ...
                  } else { safeWriteln(term, `Usage: git remote add <name> <url>`); }
                  break;
                case "pull":
                ...
                case "push": {
                  ...
                  if (result.ok) { safeWriteln(term, colorText("Push successful.", "green")); }
                  else { safeWriteln(term, colorText(`Push failed: ${result.errors ? result.errors.join(', ') : 'Unknown error'}`, "red")); }
                  break;
                }
                default:
  ...
  return (
    <Resizable defaultSize={{ width: "100%", height: "30%" }} minHeight="5%" maxHeight="90%" enable={{ top: true }} onResizeStart={() => setIsResizing(true)} onResizeStop={() => setIsResizing(false)} style={{ borderTop: isResizing ? "4px solid #4051b5" : "1px solid #333", display: showTerminal ? "flex" : "none", flexDirection: "column", backgroundColor: "#1a1a1a" }}>
      <header className={styles.terminalHeader}>
        <div className={styles.headerLeft}>
          {["PROBLEMS", "TERMINAL", "DEBUG"].map((view) => (
            <span key={view} className={`${styles.viewTab} ${ activeView === view ? styles.activeViewTab : "" }`} onClick={() => setActiveView(view)} >{view}</span>
          ))}
        </div>
        ...
        <div className={styles.terminalTabs}>
          {terminals.map((t) => (
            <div key={t.id} className={`${styles.terminalTab} ${ activeTerminalId === t.id ? styles.activeTerminalTab : "" }`} onClick={() => setActiveTerminalId(t.id)} >
              <span>{t.name}</span>
              {terminals.length > 1 && ( <X size={14} className={styles.closeTabIcon} onClick={(e) => { e.stopPropagation(); handleKillTerminal(t.id); }} /> )}
            </div>
          ))}
        </div>
